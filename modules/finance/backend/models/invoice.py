...
class InvoiceSource(models.TextChoices):
    """How a register row came to exist. `GENERATED` is the platform raising a
    bill and rendering its documents. `IMPORTED` is a historical invoice already
    issued outside the platform — scanned, read by OCR, and back-filled with its
    original printed number preserved (import never allocates a fresh number)."""
...
class Invoice(TenantOwnedModel):
    ...
    # How this row came to exist — generated by the platform, or imported from a
    # scan of an invoice already issued outside it. An import back-fills the
    # original printed number; it never allocates a new one (see numbering.py).
    source = models.CharField(
        max_length=10,
        choices=InvoiceSource.choices,
        default=InvoiceSource.GENERATED,
        db_index=True,
    )
    ...
    # The original uploaded scan of an imported invoice — the "copy for
    # analytics" kept beside the regenerated workbook/PDF. Null for generated
    # invoices, which have no external source document. SET_NULL (not PROTECT):
    # discarding the scan must never block correcting or cancelling the bill.
